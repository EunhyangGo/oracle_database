...
-- 현재는 DB 저장소가 없는 상태
-- DB에서 어떤 일을 수행하기 전에 가장 먼저 저장소를 생성해야 한다.
-- 저장소중에서 표준 DB의 대표 저장소인 TABLE을 생성하겠다.
...
CREATE TABLE tbl_addr (
    name VARCHAR2(50), -- 50글자 범위의 문자를 저장하되, 
                       -- 만약 글자수가 50글자가 안되면 남은 공간은 압축해서 저장
    age NUMBER, -- 정수 38자리
    addr VARCHAR2(125)
    );
    
    -- TABLE을 설계할때 검색하는 KEY로 사용될 가능성이 많은 항목(column), 객체에서 말하면 멤버변수들
    -- 공간 낭비를 감안하고라도 CHAR형으로 선언하는 것이 효율적이다.
    -- 숫자가 저장되는 부분
    -- NUMBER라는 키워드 만으로 설정이 가능하지만 가급적 최대자릿수를 고려하여
    -- 값을 지정해주는 것이 좋다.
...
-- 데이터 추가 테스트
-- CRUD 중의 C(CREATE) >> 이미 생성된 TABLE에 데이터 1줄 추가하는 것.
INSERT INTO tbl_addr
VALUES ('홍길동',12,'서울시 영등포구');

-- 추가된 데이터 확인 테스트
-- CRUD 중의 R(READ) >> TABLE에 저장된 데이터를 확인
...
INSERT INTO tbl_addr VALUES (4,'월매',12,'남원시');
